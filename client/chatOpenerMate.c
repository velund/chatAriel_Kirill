...
CHAT_OPENER_ERR openChat(char *_grpIp, int _grpPort, char *_userName, char *_groupName, int _chatId)
{
	...
	sprintf(listener, "mate-terminal --hide-menubar --geometry %s -t %s -- ./listen.out %s %d %s", geometryListener, _groupName, _grpIp, _grpPort, _groupName);
	sprintf(sender, "mate-terminal --hide-menubar --geometry %s -- ./send.out %s %d %s %s", geometrySender, _grpIp, _grpPort, _userName, _groupName);
